...
class LibraryController {
  ...
  Future<List<LibraryModel>> loadLibraries() async {
    ...
    if (jsonString != null) {
      ...
      return jsonList.map((jsonMap) => LibraryModel.fromJson(jsonMap)).toList();
    }
    return []; // Return an empty list if no libraries are found
  }

  // Adds a new library to the list
  Future<void> addLibrary(LibraryModel newLibrary) async {
    final libraries = await loadLibraries();
    libraries.add(newLibrary);
    await saveLibraries(libraries);
  }

  Future<void> updateLibrary(LibraryModel updatedLibrary) async {
    final libraries = await loadLibraries();
